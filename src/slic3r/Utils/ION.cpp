...
ION::ION(DynamicPrintConfig *config) :
...
bool ION::upload(PrintHostUpload upload_data, ProgressFn prorgess_fn, ErrorFn error_fn, InfoFn info_fn) const
{
    const char *name = get_name();
    ...
    http.form_add("a", "upload")
        ...
        .on_progress([&](Http::Progress progress, bool &cancel) {
            ...
            if (cancel) {
                // Upload was canceled
                BOOST_LOG_TRIVIAL(info) << "ION: Upload canceled";
                res = false;
            }
        })
        .perform_sync();
...
bool ION::validate_version_text(const boost::optional<std::string> &version_text) const
{
    return version_text ? boost::starts_with(*version_text, "ION") : true;
}

void ION::set_auth(Http &http) const
{
...
std::string ION::make_url(const std::string &path) const
{
...
bool ION::get_groups(wxArrayString& groups) const
{
    bool res = true;
...
bool ION::get_printers(wxArrayString& printers) const
{
    const char *name = get_name();
